...
def draw_plot(p: StableDiffusionProcessingTxt2Img, row_field_values, col_field_values, page_field_values, col_labels, row_labels, page_labels, cell, draw_legend, include_lone_images) -> Processed:
...
class Script(scripts.Script):
    ...
    def run(self, p, col_modules, col_values, row_modules, row_values, page_modules, page_values, draw_legend, include_lone_images, no_fixed_seeds):
        ...
        for i in range(num_col_modules):
            col_options.append(axis_options[col_modules[i]])
            col_field_values.append(process_axis(col_options[i], col_values[i]))

        for i in range(num_row_modules):
            row_options.append(axis_options[row_modules[i]])
            row_field_values.append(process_axis(row_options[i], row_values[i]))

        for i in range(num_page_modules):
            page_options.append(axis_options[page_modules[i]])
            page_field_values.append(process_axis(page_options[i], page_values[i]))
        ...
        print(f"MVar Plot Prototype will create {len(col_field_values) * len(row_field_values) * len(page_field_values) * p.n_iter} images on {len(page_field_values)} {len(col_field_values)}x{len(row_field_values)} pages. (Total steps to process: {total_steps * p.n_iter})")
        ...
        with SharedSettingsStackHelper():
            processed, page_count = draw_plot(
                p,
